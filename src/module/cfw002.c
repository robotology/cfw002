...
//	int i;
	...
	__copy_from_user((char*)&(priv->txbuf[port][priv->txidx[port]]),
...
static int cfw002_allocdesc(cfw002_priv *priv)
{
	...
	for(i=0;i<10;i++){
		...
		priv->txbuf[i] = pci_alloc_consistent(priv->pdev,
			TX_DESC_SIZE * cfw002_txdescnum, &(priv->txdma[i]));
	...
	for(i=0;i<10;i++){
		...
		priv->rxbuf[i] = pci_alloc_consistent(priv->pdev,
			RX_DESC_SIZE * cfw002_rxdescnum, &(priv->rxdma[i]));
	...
	switch(cmd &~ IOCSIZE_MASK)
	{
		...
		case CFW002_CAN_WRITE:
			...
			*/
			...
			__copy_from_user(&hdr,buf,sizeof(hdr));
		...
		//	printk("stat\n");
			...
			__copy_from_user(&port,buf,sizeof(u8));
...
static long cfw002_firmwareload(cfw002_priv *priv, const struct firmware *fw)
{
	...
	printk(KERN_INFO "%s (cfw002): uploading %d bytes firmware\n",
...
static int cfw002_debugfs_read(struct file *filp, char __user *buff, size_t count, loff_t *offp)
{
	cfw002_priv *priv;
	int num;
